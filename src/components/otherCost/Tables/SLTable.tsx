import AddImage from "@/assets/icons/AddImage"
import SubtractImage from "@/assets/icons/SubstractImage";
import { deleteOtherCost, fetchSoftwareCategories, getOtherCost, getSoftwareLicenseTotal, getslRow, setSoftwareLicenseTotal, setslRow } from "@/features/budget/otherCostDiscountSlice";
import { useDispatch, useSelector } from 'react-redux';
import { AppDispatch } from "@/features/store";
import { useSearchParams } from "next/navigation";
import { useEffect, useState } from "react";
import Confirmation from "@/components/common/Modal/Confirmation";

interface SlTableData
 {
    otherCostId?: number;
    hardwareSoftwareLicenseFee: number,
    softwareLicenseModelId : number,
    unitCost: number,
    noOfUnits: number
}

const SLTable = ({setBackendData} :any) => {
    const searchParams = new URLSearchParams(useSearchParams().toString());
    const projectId: any = searchParams.get('projectId');
    const dispatch = useDispatch<AppDispatch>()
    const options = useSelector(fetchSoftwareCategories)
    const row :Array<SlTableData> = useSelector(getslRow)
    const total = useSelector(getSoftwareLicenseTotal)
    const [open, setOpen] = useState(false)
    const [confirm, setConfirm] = useState(false)
    const [deleteData, setDelete] = useState(0)
    
    useEffect(() => {
        if(confirm){
            dispatch(deleteOtherCost({projectId: projectId, otherCostId: deleteData})).then((res :any)=>{
                dispatch(getOtherCost(projectId)).then((res :any)=>{
                    setBackendData(res.payload)
                    if(res?.payload?.softwareLicense.length > 0)
                        dispatch(setslRow(res?.payload?.softwareLicense))
                    else
                        dispatch(setslRow([{hardwareSoftwareLicenseFee: 0, softwareLicenseModelId: 0, unitCost: 0, noOfUnits:0}]))
                    dispatch(setSoftwareLicenseTotal(res?.payload?.softwareLicenseAmount))
                })
                alert("Deleted Successfully")
            })
        }
    }, [confirm])
    

    const handleTotal = (row: Array<SlTableData>) => {
        let tot :number=0
        row.map((obj: any) => tot+= isNaN(obj.hardwareSoftwareLicenseFee+(obj.unitCost * obj.noOfUnits)) 
                                        ? 0 : Number(obj.hardwareSoftwareLicenseFee+(obj.unitCost * obj.noOfUnits)))
        dispatch(setSoftwareLicenseTotal(tot))
    }

    const handleUnit = (value :string, index: number) => {
        const updatedArray = row.map((object: SlTableData, ind: number) => {
            const obj = {...object}
            if(ind == index)
                obj.noOfUnits = isNaN(parseInt(value)) || value == undefined ? 0 : parseInt(value)
            return obj
        })
        dispatch(setslRow(updatedArray))
        handleTotal(updatedArray)
    }

    const handleCost = (value :string, index: number) => {
        const updatedArray = row.map((object: SlTableData, ind: number) => {
            const obj = {...object}
            if(index == ind)
                obj.unitCost = isNaN(parseInt(value)) || value == undefined ? 0 : parseInt(value)
            return obj
        })
        dispatch(setslRow(updatedArray))
        handleTotal(updatedArray)
    }

    const handleFee = (value :string, index: number) => {
        const updatedArray = row.map((object: SlTableData, ind: number) => {
            const obj = {...object}
            if(ind == index)
                obj.hardwareSoftwareLicenseFee = isNaN(parseInt(value)) || value == undefined ? 0 : parseInt(value)
            return obj
        })
        dispatch(setslRow(updatedArray))
        handleTotal(updatedArray)
    }

    const handleModel = (value :number, index: number) => {
        const updatedArray = row.map((object: SlTableData, ind: number) => {
            const obj = {...object}
            if(ind == index)
                obj.softwareLicenseModelId = value
            return obj
        })
        dispatch(setslRow(updatedArray))
    }

    const addItem = () => {
        dispatch(setslRow([...row,{hardwareSoftwareLicenseFee: 0, softwareLicenseModelId: 0, unitCost: 0, noOfUnits:0}]))
    }

    const removeItem = async(index: number) => {
        const object = row.find((_,ind)=> ind == index)
        if(object && compareData(object) && row.length == 1)
            alert("Please enter values")
        else{
            if(object && object.otherCostId){
                setDelete(object?.otherCostId)
                setOpen(true)
                setConfirm(false)
            }
            else{
                dispatch(setSoftwareLicenseTotal(total-(row[index].hardwareSoftwareLicenseFee+(row[index].unitCost * row[index].noOfUnits))))   
                const updatedArray = row.filter((_,ind)=> ind !== index)
                dispatch(setslRow(updatedArray))
                if(row.length == 1)
                    dispatch(setslRow([{hardwareSoftwareLicenseFee: 0,softwareLicenseModelId: 0,unitCost: 0, noOfUnits:0}]))
            }
        }
    }

    const compareData = (object: SlTableData) => {
        return object.hardwareSoftwareLicenseFee == 0 && object.softwareLicenseModelId == 0 && object.unitCost == 0 && object.noOfUnits == 0 
    }

    return(
        <>
        <div className="relative justify-center rounded-lg overflow-hidden">
            <table className='table-auto'>
                <thead className='bg-[#1D154ACC]'>
                    <tr>
                        <th>Hardware / Software<br />License Fee</th>
                        <th>Subscription<br />Model</th>
                        <th>Charge Per <br /> Unit</th>
                        <th>No. of Time <br />Units</th>
                        <th>Total Cost</th>
                        <th></th>
                    </tr>        
                </thead>
                <tbody className='bg-[#4B3D99B8]'>
                    {
                        row?.map((object:any, index:number)=>{
                            return(
                                <tr>
                                    <td className='p-3'>
                                        <input onChange={(event)=>handleFee(event.target.value,index)} 
                                        className='bg-[#01105F] rounded-md p-1 pl-2' placeholder='Enter License Fee'
                                        value={object.hardwareSoftwareLicenseFee == 0 ? "" : object.hardwareSoftwareLicenseFee}
                                        type="number" />
                                    </td>
                                    <td className='p-3'>
                                        <select onChange={(event)=>handleModel(parseInt(event.target.value), index)}
                                            value={object.softwareLicenseModelId}
                                            className='bg-[#01105F] p-2 rounded-md' name="model">
                                            <option value={0}>Select Model</option>
                                            {
                                                Array.isArray(options) && options?.map((element :any) => {
                                                    return (
                                                        <option value={element.id}>{element.name}</option>
                                                    )
                                                })
                                            }
                                        </select>
                                    </td>
                                    <td className='p-3'>
                                        <input onChange={(event)=>handleCost(event?.target.value, index)} 
                                        className='bg-[#01105F] rounded-md w-24 p-1 text-center' type="number" 
                                        value={(object.unitCost==0)?"":object.unitCost}/>
                                    </td>
                                    <td className='p-3'>
                                        <input onChange={(event)=>handleUnit(event?.target.value, index)} 
                                        className='bg-[#01105F] rounded-md w-24 p-1 text-center' type="number" 
                                        value={(object.noOfUnits==0)?"":object.noOfUnits}/>
                                    </td>
                                    <td className='p-3'>
                                        <div className='bg-[#01105F] rounded-md w-24 p-1 text-center truncate'>
                                            {isNaN(object.hardwareSoftwareLicenseFee+(object.unitCost * object.noOfUnits)) ? 0 : object.hardwareSoftwareLicenseFee+(object.unitCost * object.noOfUnits)}
                                        </div>
                                    </td>
                                    <td className='p-3 px-6 text-[#4B3D99B8]' onClick={()=>removeItem(index)}>
                                        <SubtractImage width={20} height={20} color={"white"}/>
                                    </td>
                                </tr>
                            )
                        })
                    }
                </tbody>
            </table>
        </div>
            <div className="absolute bottom-1" onClick={()=>addItem()}>
                <AddImage width={20} height={20} color={"#01105F"}/>
            </div>
            <Confirmation 
            open={open}
            setOpen={setOpen}
            data='Any unsaved data will be gone. Are you sure?'
            setConfirm={setConfirm}
        />
        </>
    )
}

export default SLTable